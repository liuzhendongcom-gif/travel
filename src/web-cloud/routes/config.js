...
const { getFullConfig, saveFullConfig } = require('../token-store');
const { maskToken } = require('../../main/security');
...
router.post('/settings', (req, res) => {
  try {
    ...
    const { maskToken: _mask, validateToken } = require('../../main/security');

    // token 验证：含 *** 为掩码（保留原值）；否则必须 ≥8 字符
